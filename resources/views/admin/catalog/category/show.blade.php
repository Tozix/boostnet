...
@if ($category->children->count())
    ...
    <table class="table table-bordered">
        ...
        <tr>
            ...
            <td>
                <a href="{{ route('admin.catalog.category.show', ['category' => $child->id]) }}">
                    {{ $child->name }}
                </a>
            </td>
            <td>{{ $child->slug }}</td>
            <td>
                <a href="{{ route('admin.catalog.category.edit', ['category' => $child->id]) }}">
                    <i class="far fa-edit"></i>
                </a>
            </td>
            <td>
                <form action="{{ route('admin.catalog.category.destroy', ['category' => $child->id]) }}" method="post">
                    @csrf
                    @method('DELETE')
...
@endif
<a href="{{ route('admin.catalog.category.edit', ['category' => $category->id]) }}" class="btn btn-success">
    Редактировать категорию
</a>
<form method="post" class="d-inline" action="{{ route('admin.catalog.category.destroy', ['category' => $category->id]) }}">
    @csrf
    @method('DELETE')
